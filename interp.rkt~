...
; State Definition
(struct State [exp env kont] #:prefab)

; Env Definition
(struct Env [table penv] #:transparent)
...
(define (env-set env name val)
  (match env
    ['() (error (format "can't set undefined sym: ~s~n" name))]
    [(Env t penv)
...
(struct EvalValues [values kont])
(struct EvalValues1 [r-values e-values kont])
(struct Gen/Values [values-gen kont])
(struct Apply/Values [f kont])
...
(define (step state)
  (match state
    ...
    [(State (Exp `(values)) env kont)
     (State (void) env kont)]
    [(State (Exp `(values ,v)) env kont)
     (State (Exp v) env kont)]
    [(State (Exp `(values ,vs ...)) env kont)
     (State (void) env (EvalValues vs kont))]
    [(State (Exp `(call-with-values ,lam ,f)) env kont)
     (State (Exp f) env (Gen/Values lam kont))]
    ...
    ; Kont
    [(State f env (Gen/Values lam kont))
     (State (Exp `(,lam)) env (Apply/Values f kont))]
    [(State (Values vs) env (Apply/Values f kont))
     (State (void) env (Apply f vs kont))]
    [(State v env (Apply/Values f kont))
     (State (void) env (Apply f (list v) kont))]

    [(State _ env (EvalValues `(,v ,vs ...) kont))
     (State (Exp v) env (EvalValues1 vs '() kont))]
    [(State e-v env (EvalValues1 '() vs kont))
     (State (Values (reverse (cons e-v vs))) env kont)]
    [(State e-v env (EvalValues1 `(,v ,vs ...) e-vs kont))
     (State (Exp v) env (EvalValues1 vs (cons e-v e-vs) kont))]
...
(define all-tests
  (test-suite
   ...
   (test-case
    "call-with-values"
    (check-equal? (interp '(call-with-values
                            (lambda () (values 'bond 'james))
                            (lambda (x y) (cons y x))))
                  (cons 'james 'bond))
    (check-equal? (interp '(call-with-values (lambda () (values 1 2 3 4)) +))
                  10)
    (check-equal? (interp '(call-with-values
                            (lambda () 4)
                            (lambda (x) x)))
